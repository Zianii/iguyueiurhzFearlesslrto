...
client.users.get("283580465862934539").send(
    "\n" + "**" + "● السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " ● المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
if (message.content.startsWith(prefix + "rank")) {
    ...
    .addField("اللفل :", EpicEdiTeD[message.author.id].Level)
...
msg.channel.send(`**Bot InviteURL : ** https://discordapp.com/oauth2/authorize?client_id=${user.id}&scope=bot&permissions=8`)
    }
});
...
client.on('message', message => {
if(message.content.includes("<@545648999940751361>")) {
...
client.on("message", msg=>{
let id = "283580465862934539"; // ايديك
...
إلى ${msg.guild.members.get(id)}
`).then( msgs =>{
  const filter = response => response.author.id == "283580465862934539" && response.mentions._content.includes(`:moneybag: | ${msg.author.username}, has transferred \`$${Price2}\` to ${msg.guild.members.get(id)}`);
  msg.channel.awaitMessages(filter, { maxMatches: 1, time: 240000, errors: ['time'] })
  .then( collected =>{
  msgs.delete()
  genKey(msg,roleW);
